    ...
    const handleSubmitSubTypes = async (event) => {
        ...
        try {
            ...
            if (response.status === 200) {
                alert('Subcategory added successfully!');
                setShowForm(false);
                setSubTypes([...subTypes, response.data.subTypes]);
                setImagePreview('');
        ...
        <center>
            <div className="subcategory-list">
                ...
                <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
                    {subTypes.map((subType) => (
                        <div key={subType._id} style={{ margin: 10 }}>
                            <span onClick={() => navigate(`./Size/${subType._id}`)} style={{ cursor: 'pointer' }}>
                                <img
                                    src={subType.imageUrl ? `${server}/${subType.imageUrl}` : 'default-image.png'}
                                    alt={subType.name}
                                    style={{ objectFit: 'contain' }}
                                    height={100}
                                    width={100}
